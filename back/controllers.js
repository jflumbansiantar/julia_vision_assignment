...
class controller {
    static async Login(req, res, next) {
        ...
        try {
            ...
            user.forEach(data => {
                if (data.username !== username) res.status(404).json({ status: 404, message: `User not found` });

                if (decryptPassword(password, data.password)) {
                    const token = tokenGenerator(user);

                    res.status(200).json({ status: 200, message: "Successfully logged in!", username: data.username, token: token });
                }
            })
        } catch (error) {
    ...
    static async Public(req, res, next) {
        try {
            res.status(200).json({ status: 200, message: "Public Area!" });
        } catch (error) {
            res.status(500).json({ status: 500, message: `Internal Server Error` });
        }
    };
    ...
    static async Private(req, res, next) {
        ...
        try {
            ...
            if(!token) {
                res.status(404).json({ status: 404, message: "Token is not found!" });
            } else {
                const decoded = tokenVerifier(token);

                if (decoded.username === user.username) {
                    res.status(200).json({ status: 200, message: "Token is decoded", data: decoded });            
                } else {
